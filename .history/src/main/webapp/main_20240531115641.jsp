...
<body>
    <div class="today" style="height: 100%;">
        <div id="todays-tasks-form" style="height: auto;">
            ...
            <%
                ...
                if(completedTasks.size() == 0) { %>
                    <input type="hidden" name="completedTasks" id="completedTasksSize" value="0">
                <%   System.out.println("Completed task list size:"+completedTasks.size()); 
                }else{
                    System.out.println("no completed tasks"); %>
                    <input type="hidden" name="tasks" id="tasksSize" value="1">
                <% }

                if(tasks.size() == 0) { %>
                    <input type="hidden" name="tasks" id="tasksSize" value="0">
            ...
            <h1 id="today-head">Tasks</h1>
            <input type="button" id="add-task" value="+ add task"><br>
            <div id="incomplete" class ="tasks" id="incomplete">

                
                <% System.out.println("reached line 38"); %>

                <% if (tasks.size() > 0) { %>
                    <h1 style="font-weight: 500;" id="incomplete-heading">Incomplete</h1>
                   <% for (Task task : tasks) { %>
                        <form id="task-form" action="complete" method="post">
                            <form id="task-p-form" action="complete" method="post">
                                ...
                                <button id="complete-task" type="submit" class="task-buttons">Complete</button>
            ...
            <div id="completed" class ="tasks" id="complete">
                <% System.out.println("reached line 63"); %>
                <% if (completedTasks.size() > 0) { %>
                    <h1 style="font-weight: 500;" id="complete-heading">Completed</h1>
                   <% for (Task completedTask : completedTasks) { %>
                    <form id="completed-task-form" action="undo" method="post">
                        <div class="task-display">
                        <p id="task-p"><%= completedTask.getTaskName() %></p>
                        <button id="undo-task" type="submit" class="task-buttons">Undo</button>
                        ...
                        <form id = "duplicate-task-form" action="duplicate" method="post">
                            <button id="duplicate" type="submit" class="task-buttons">Duplicate</button>
                            <input type="hidden" name="taskName" id="taskName" value="<%= completedTask.getTaskName() %>">
                        </form>
                        </div>
                    </form>
        ...
        <form class="menu">
            ...
            <div id="list-section" >
                ...
                <h1 id="list-heading" style="position: relative; left: 30%;">categories</h1>
                <ul id="lists">
                    <% if (categories != null) {
                        for (String category : categories) { %>
                            <form id="filter-form-<%= category %>">
                            <input type="button" id="list-button" value="<%= category %>" class="categories">
                            <input type="hidden" name="category" id="category" value="<%= category %>">
                        </form>
                <%  }
                } %>
                <form id="getAllTasks" action="get-tasks" method="get">
                    <input type="submit" id="list-button" value="all" class="categories">
            ...
            <div id="misc">
                <a href=/Todo/settings id="settings" style="text-decoration: none;">☰ Settings</a>
                <p><a href="logout" sty>⬅ Logout</a></p>
            </div>
        </form>
        ...
        <form class="task" id = "edit-task-form" style="display: none;" >
            ...
            <ul id="task-items">
                <input type="text" id="edit-task-name" name="edit-task-name" placeholder="task" style="position: relative; left: 2%;">
            </ul>
            ...
            <div id="save-delete">
                ...
                <input type="button" id = "save-task" name="save" value="Save Task">
            </div>
        </form>
